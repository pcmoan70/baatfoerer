...
def main() -> int:
    ...
    for im in ana["images"]:
        cur.execute("""INSERT OR IGNORE INTO images(file,site,category,norwegian_term,what_it_shows,reusable,page_url,license,attribution,flashcard)
                       VALUES(?,?,?,?,?,?,?,?,?,?)""",
                    (im["file"], im["file"].split("/")[0], im.get("category"),
                     ...
                     0 if im.get("flashcard") is False else 1))
